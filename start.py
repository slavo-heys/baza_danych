...
try:
    ...
    df = pd.DataFrame(columns=['data', 'imie', 'nazwisko', 'pesel', 'wiek', 'plec', 'miasto', 'kod', 'ulica', 'nr_budynku',
...
# --------------------------------------------------------------------------------
        ...
        df.loc[len(df)] = [self.data, self.imie, self.nazwisko, self.pesel, self.wiek, self.plec, self.miasto, self.kod, self.ulica,
